...
public class ElementsByType {
    ...
    void recursiveSortElements(Collection<? extends GraphElementModel> graphElementModels) {
        for (var element : graphElementModels) {
            ...
            if (element instanceof StickyNoteModel stickyNoteModel) stickyNoteModels.add(stickyNoteModel);
            else if (element instanceof PlacematModel placematModel) placematModels.add(placematModel);
            ...
            else if (element instanceof GroupModel groupModel) groupModels.add(groupModel);
            else if (element instanceof WireModel wireModel) wireModels.add(wireModel);
            else if (element instanceof WireReroutePointModel reroutePointModel) wireReroutePointModels.add(reroutePointModel);
            else if (element instanceof AbstractNodeModel nodeModel) nodeModels.add(nodeModel);
        }
    }
}
