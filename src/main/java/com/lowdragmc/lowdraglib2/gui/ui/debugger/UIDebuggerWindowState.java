    ...
    private static Optional<UIDebuggerWindowState> read() {
        ...
        try {
            ...
            return Optional.of(new UIDebuggerWindowState(
                    ...
                    tag.contains("y") ? tag.getInt("y") : UNPLACED,
                    ...
                    // finding this file.
                    Math.max(MIN_WIDTH, tag.contains("width") ? tag.getInt("width") : DEFAULT.width),
                    Math.max(MIN_HEIGHT, tag.contains("height") ? tag.getInt("height") : DEFAULT.height),
                    tag.getBoolean("alwaysOnTop")));
        } catch (Exception e) {
