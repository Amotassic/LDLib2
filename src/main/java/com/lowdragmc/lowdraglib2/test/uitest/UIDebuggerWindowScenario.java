...
public class UIDebuggerWindowScenario implements UIScenario {
    ...
    /** F3, then the title bar's window toggle — the two clicks a user makes to get a window. */
    static ScenarioBuilder popOut(ScenarioBuilder s) {
        return s.key(GLFW.GLFW_KEY_F3)
                .frames(10)
    ...
    private static boolean hasRedNear(NativeImage frame, float guiX, float guiY, double scale) {
        ...
        for (int y = from; y < to; y++) {
            // getPixelRGBA hands back the raw ABGR word, not ARGB - read it through the right lens or
            // "red" silently means "blue".
            var pixel = frame.getPixelRGBA(x, y);
            if (FastColor.ABGR32.red(pixel) > 200
                    && FastColor.ABGR32.green(pixel) < 60
                    && FastColor.ABGR32.blue(pixel) < 60) {
                return true;
            }
        }
        return false;
