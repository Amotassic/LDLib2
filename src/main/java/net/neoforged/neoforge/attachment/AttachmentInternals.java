...
public final class AttachmentInternals {
     ...
     */
    private static void copyAttachments(HolderLookup.Provider provider, IAttachmentHolder fromO, IAttachmentHolder toO, Predicate<AttachmentType<?>> filter) {
        var from = fromO.getAttachmentHolder();
        if (from.attachments == null) {
            return;
        }
        var to = toO.getAttachmentHolder();
    ...
    public static void copyChunkAttachmentsOnPromotion(HolderLookup.Provider provider, IAttachmentHolder from, IAttachmentHolder to) {
