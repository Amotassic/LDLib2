...
public class LevelAttachmentsSavedData extends SavedData {
    ...
    public LevelAttachmentsSavedData(ServerLevel level, CompoundTag tag) {
        this.level = level;
        level.getAttachmentHolder().deserializeAttachments(level.registryAccess(), tag);
    }

    @Override
    public CompoundTag save(CompoundTag tag) {
        // Make sure we don't return null
        return Objects.requireNonNullElseGet(level.getAttachmentHolder().serializeAttachments(level.registryAccess()), CompoundTag::new);
    }
